...
// version
var s = $1("curl -L https://api.github.com/repos/txthinking/httpserver/releases/latest")
var v = JSON.parse(s).tag_name
await writefile(`${nami.cache_dir}/version`, v)

// download and copy command files into nami.cache_dir
cp(`https://github.com/txthinking/httpserver/releases/latest/download/httpserver_${nami.os}_${nami.arch}${nami.os == "windows" ? '.exe' : ''}`, `${nami.cache_dir}/httpserver${nami.os == "windows" ? '.exe' : ''}`)
